...
static ProcessResult process_exec_internal(const char *cwd, char *const argv[], bool force_color)
{
	...
	if (pid == 0) {
		...
		if (force_color && !getenv("NO_COLOR")) {
...
{
	return process_exec_internal(cwd, argv, false);
}

ProcessResult process_exec_colored(const char *cwd, char *const argv[])
{
	return process_exec_internal(cwd, argv, true);
}
