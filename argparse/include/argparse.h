...
typedef struct ArgCommand {
	...
	/*
	 * Parent in the command tree. Top-level commands (added via
	 * argparse_add_command) point at &parser->root; deeper subcommands
	 * point at whichever command they were added under. Only the root
	 * itself has parent == NULL. This lets option lookup and help text
	 * walk the whole tree uniformly, regardless of nesting depth.
	 */
